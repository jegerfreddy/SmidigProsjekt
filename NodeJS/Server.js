...
app.get("/api/winningEvent/:actEventId/new", async (req, res) => {
    ...
    try {
        const response = await axios.get(`http://localhost:8080/api/actEvent/next/${actEventId}`, getAxiosConfig());
...
app.get("/api/actEvent/next/:actEventId/:option", async (req, res) => {
    ...
    try {
        const response = await axios.get(`http://localhost:8080/api/actEvent/next/${actEventId}/${option}`, getAxiosConfig());
        res.status(200).json(response.data);
    } catch (error) {
        console.error("Error linking events:", error);
        res.status(500).json({ error: 'Failed to link events' });
...
app.get("/api/adminUser/checkUsername/:id", async (req, res) => {
    const { id } = req.params;
    try {
        const response = await axios.get(`http://localhost:8080/api/adminUser/checkUsername/${id}`, getAxiosConfig());
...
app.post("/api/addNewAdmin", async (req, res) => {
    ...
    try {
        const response = await axios.post("http://localhost:8080/api/adminUser/new", newAdmin, getAxiosConfig());
...
app.put("/api/updateAdmin", async (req, res) => {
    const data = req.body;
    try {
        await axios.put("http://localhost:8080/api/adminUser/update", data, getAxiosConfig());
    } catch (error) {
        console.log(error);
    };
});
...
app.post("/api/loginAdmin", async (req, res) => {
    const data = req.body;
    try {
        const response = await axios.post('http://localhost:8080/api/adminUser/loginAdmin', data, getAxiosConfig());
...
app.get("/api/getActs", async (req, res) => {
    try {
        const dbRes = await axios.get("http://localhost:8080/api/act/all", getAxiosConfig());
        res.send(dbRes.data);
    } catch (error) {
...
app.get("/api/getEvents", async (req, res) => {
    try {
        const dbRes = await axios.get("http://localhost:8080/api/actEvent/all", getAxiosConfig());
        res.send(dbRes.data);
    } catch (error) {
...
app.get("/api/getAllEventsFromAct/:id", async (req, res) => {
    const { id } = req.params;
    try {
        const dbRes = await axios.get(`http://localhost:8080/api/actEvent/act/${id}`, getAxiosConfig());
        res.send(dbRes.data);
    } catch (error) {
        console.error('Error occurred while fetching events:', error);
        res.status(500).json({ error: 'Failed to fetch events' });
...
app.post("/api/updateEvent", async (req, res) => {
    const data = req.body;
    try {
        await axios.put("http://localhost:8080/api/actEvent/update", data, getAxiosConfig());
        res.send();
    } catch (error) {
