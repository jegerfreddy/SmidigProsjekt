...
const ChooseAvatarPage: React.FC = () => {
    ...
    return (
        <div className='position-relative vh-100 bgColor'>
            <section className={`text-center my-4 position-absolute ${orientation === 'vertical' ? 'top-0 start-50 translate-middle-x vertical-layout' : 'top-0 end-50 m-4 horizontal-layout'}`}>
                <h2>Velg avatar</h2>
            </section>
            ...
            <section className={`text-center position-absolute ${orientation === 'vertical' ? 'top-50 start-50 mb-5 translate-middle vertical-layout' : 'bottom-0 start-0 mb-2 horizontal-layout'}`}>
                <AvatarList onClick={handleClick} orientation={orientation} />
            </section>

            <section>
                <button className={`position-absolute pinButton ${orientation === 'vertical' ? 'bottom-0 start-50 translate-middle-x mb-4 vertical-layout' : 'bottom-0 end-0 m-5 mb-4 horizontal-layout'}`} onClick={() => handleSubmit({ avatarNumber, username: username ?? '' })}>Fortsett</button>
            </section>
